/**
 * @Function: Double-differenced pseudorange residual block with differencing
...
template <int... Ns>
MultiPseudorangesErrorDD<Ns...>::MultiPseudorangesErrorDD(
  ...
  // Group 2
  else if (dims_.kNumParameterBlocks == 2 && dims_.GetDim(0) == 7 &&
           dims_.GetDim(1) == 3) {
    is_estimate_body_ = true;
    is_estimate_atmosphere_ = false;
    parameter_block_group_ = 2;
  } else {
    LOG(FATAL) << "MultiPseudorangesErrorDD parameter blocks setup invalid!";
  }
...
// Set the information.
template <int... Ns>
void MultiPseudorangesErrorDD<Ns ...>::setInformation(
    const GnssErrorParameter &error_parameter) {
  ...
  var_sd_base = (square(factor(0)) + square(factor(1) / sin(elev_rov_base))) *
                ratio * 2.0;
  ...
  for (size_t i = 0; i < num_residuals(); i++) {
    ...
    var_sd = (square(factor(0)) + square(factor(1) / sin(elevation_rov))) *
             ratio * 2.0;
  ...
  CHECK(lltOfCovariance.info() == Eigen::Success)
      << "DD multi-pseudoranges covariance - LLT decomp. fail";
  whitening_cholesky_factor_ = lltOfCovariance.matrixL();
}

template <int... Ns>
bool MultiPseudorangesErrorDD<Ns ...>::Evaluate(double const *const *parameters,
                                           double *residuals,
...
template <int... Ns>
bool MultiPseudorangesErrorDD<Ns...>::EvaluateWithMinimalJacobians(
  ...
  for (size_t i = 0; i < num_residuals(); i++) {
    ...
    double dpseudorange_estimate =
        rho_rov - rho_ref - rho_rov_base + rho_ref_base;
